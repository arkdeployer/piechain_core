...
echo 'alias pi="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/arkdeployer/piechain_core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/arkdeployer/piechain_core'"
...
rm -rf "$HOME/.config/@pi"
rm -rf "$HOME/.config/@piechain"
rm -rf "$HOME/.config/pi-core"
